...
class ObjectiveFunctionPairwiseWithTotal(ObjectiveFunction):
    def __call__(self, labels, attr):
        ...
        obj_val = np.var(regionpops) + sum(
            ...
            for r in regions_set
            for i, j in itertools.combinations(np.where(labels == r)[0], 2))
#        print('fraction due to pop', np.var(regionpops)/obj_val)
        return obj_val
    ...
    def update(self, moving_area, recipient_region, labels, attr):
        ...
        donor_diff = sum(
            self.metric(attr_donor, attr[moving_area,1:].reshape(1, -1)))
        ...
        recipient_diff = sum(
            self.metric(attr_recipient, attr[moving_area,1:].reshape(1, -1)))
        
        varchange = -2*moving_popu*(pop_donor - pop_recipient- moving_popu)/len(set(labels)) 
