...
#            if self.n != sum(cardinality):
    ...
    def grow_compact(self, w, test_card, regst, candidates, potential):
    ...
    def grow_free(self, w, test_card, regst, candidates, potential):
        ...
        potential.extend([i for i in w.neighbors[add_area]
                             if i not in regst.regions[-1] and
                                i not in potential and
                                i in candidates and
                                self.region_size_check(regst.getwt() + self.wtdict[i] , test_card)!=1
                                ])
    ...
    def build_contig_regions(self, num_regions, cardinality, w,
        ...
        while iter < maxiter:
            ...
            rsaccept = RegStruct(self.area_ids, [], self.wtdict)
            size_pre = 0
            counter = -1
            self.feasible = False
            swap_count = 0
...
#                    print([sum([self.wtdict[i] for i in regions[j]]) for j in range(len(regions))]) 
            ...
            if rs.n < num_regions:
                ...
                self.feasible = True
                iter = maxiter
        self.regions = rsaccept.regions

class RegStruct(dict):
    
    def __init__(self, areas, region_list = [], wtdict= None):
        dict.__init__({area: None for area in areas})
        self.regions = [i.copy() for i in region_list]
        self.n = len(region_list) 
        self.update({ area: regionind  for regionind in range(self.n) for area in region_list[regionind] })
        if wtdict is None:
    ...
    def area_into_region(self, area, regionind):
        ...
        if area in self.keys():
            ...
            if self[area] is not None:
                self.regions[self[area]].remove(area)
                self.regionwts[self[area]] -= self.wtfun(area)
        #include in new region
        self[area] = regionind
        self.regions[regionind].append(area)
        self.regionwts[regionind] += self.wtfun(area)

    def droplast(self):
        self.regionwts.pop()
        for i in self.regions[-1]:
            self.pop(i)
        return self.regions.pop()
    
    def getwt(self, regindex= -1):
        return self.regionwts[regindex]
